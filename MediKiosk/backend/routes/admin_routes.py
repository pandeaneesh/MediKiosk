...
HOSPITAL_ALLOWED_PASSWORDS = ["hosp@1234", "hosp1234", "hospital123", "Admin@123", "admin123"]
GOVT_ALLOWED_PASSWORDS = ["govt@1234", "govt1234", "gov123", "GovAdmin@2026", "director123"]

def ensure_default_users():
    try:
        default_users = [
            ('usr-admin-hosp', 'hospital.admin@medikiosk.gov.in', 'hospital.admin@medikiosk.gov.in', 'HOSPITAL_ADMIN', 'hosp@1234', 'Dr. V. K. Paul (Hospital Medical Superintendent)', 'HOSP-AIIMS-01'),
            ('usr-admin-aiims', 'admin@aiims.edu', 'admin@aiims.edu', 'HOSPITAL_ADMIN', 'Admin@123', 'AIIMS Medical Superintendent', 'HOSP-AIIMS-01'),
            ('usr-admin-test-hosp', 'admin.medikiosk@gmail.com', 'admin.medikiosk@gmail.com', 'HOSPITAL_ADMIN', '12345', 'AIIMS Administrator', 'HOSP-AIIMS-01'),
            ('usr-admin-sjh', 'sjh-del.admin@medikiosk.gov.in', 'sjh-del.admin@medikiosk.gov.in', 'HOSPITAL_ADMIN', 'sjh-del@1234', 'Dr. S. K. Mahapatra (Medical Superintendent, Safdarjung)', 'HOSP-SJ-02'),
            ('usr-admin-pune', 'admin.civilpune@medikiosk.gov.in', 'admin.civilpune@medikiosk.gov.in', 'HOSPITAL_ADMIN', 'pune@1234', 'Dr. Ravindra Patil (Civil Surgeon, Pune District Hospital)', 'HOSP-CIVIL-03'),
            ('usr-admin-kmc', 'kmc-mnp.admin@medikiosk.gov.in', 'kmc-mnp.admin@medikiosk.gov.in', 'HOSPITAL_ADMIN', 'kmc-mnp@1234', 'Dr. Ramesh Kamath (Superintendent, Kasturba Udupi)', 'HOSP-KMC-04'),
            ('usr-admin-govt', 'govt.stakeholder@abdm.gov.in', 'govt.stakeholder@abdm.gov.in', 'MAIN_ADMIN', 'govt@1234', 'Er. Sachin Bansal (National Health Authority & Stakeholders Director)', None),
            ('usr-admin-mohfw', 'director@mohfw.gov.in', 'director@mohfw.gov.in', 'MAIN_ADMIN', 'GovAdmin@2026', 'Director General Health Services (MoHFW)', None),
            ('usr-admin-test-gov', 'gov.director@abdm.gov.in', 'gov.director@abdm.gov.in', 'MAIN_ADMIN', '12345', 'National Health Authority Director', None)
        ]
...
def admin_common_login(req: AdminLoginRequest):
    ...
    if is_govt_target:
        ...
        is_valid = (user and user.get("password_hash") == clean_pass) or (clean_pass in GOVT_ALLOWED_PASSWORDS) or (clean_pass == "12345")
        if not is_valid and clean_pass in ["hosp@1234", "Admin@123"]:
            raise HTTPException(
                status_code=401,
        ...
        if clean_pass in ["govt@1234", "GovAdmin@2026"]:
            raise HTTPException(
                status_code=401,
        ...
        # Match hospital in database
        matched_hospital = None
        h_rows_all = execute_query("SELECT * FROM hospitals")
        for h in h_rows_all:
            h_code = (h.get("code") or "").lower()
            h_clean = h_code.replace("-", "")
            h_id = (h.get("hospital_id") or "").lower()
            if clean_user in [f"{h_code}.admin@medikiosk.gov.in", f"{h_clean}.admin@medikiosk.gov.in", "admin.civilpune@medikiosk.gov.in", "hospital.admin@medikiosk.gov.in"] or h_id in clean_user or h_clean in clean_user:
                matched_hospital = h
                break

        is_valid = False
        if user and user.get("password_hash") == clean_pass:
            is_valid = True
        elif clean_pass in HOSPITAL_ALLOWED_PASSWORDS or clean_pass == "12345":
            is_valid = True
        elif matched_hospital:
            expected_p1 = f"{(matched_hospital.get('code') or '').lower()}@1234"
            expected_p2 = f"{(matched_hospital.get('code') or '').lower().replace('-', '')}@1234"
            if clean_pass in [expected_p1, expected_p2, "pune@1234", "hosp@1234", "hosp1234"]:
                is_valid = True
        ...
        role = "HOSPITAL_ADMIN"
        full_name = user["full_name"] if user else (f"Dr. Medical Superintendent ({matched_hospital['name']})" if matched_hospital else "Dr. V. K. Paul (Hospital Medical Superintendent)")
        hospital_id = user["hospital_id"] if (user and user.get("hospital_id")) else (matched_hospital["hospital_id"] if matched_hospital else "HOSP-AIIMS-01")
        hospital_name = matched_hospital["name"] if matched_hospital else "AIIMS New Delhi Central Hospital"
