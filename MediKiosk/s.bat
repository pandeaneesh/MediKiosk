...
for /f "tokens=5" %%a in ('netstat -aon 2^>nul ^| findstr ":8000 :5173 :5175" ^| findstr "LISTENING"') do (
...
echo [2/4] Initializing Edge SQLite Database and Starting Python Backend Server...
where python >nul 2>&1
if errorlevel 1 (
    echo       [!] Python not detected in PATH. Please install Python 3.9+ to enable backend AI and API services.
    goto backend_done
)
...
echo ====================================================================
echo.
echo  [1] LOCAL WORKSTATION (Main Kiosk Portal):
echo      http://localhost:5173
echo.
echo  [2] DIRECT ACCESS MODES:
echo      - Patient Kiosk Portal:    http://localhost:5173/?mode=patient
echo      - Doctor OPD Chamber:      http://localhost:5173/?mode=doctor
echo      - Admin Command Center:    http://localhost:5173/?mode=admin
echo.
echo  [3] LOCAL NETWORK / TOUCHSCREEN KIOSK / TABLET:
echo      http://%LOCAL_IP%:5173
echo.
echo  [4] BACKEND EDGE API:
