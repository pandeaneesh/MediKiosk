...
// Universal Multilingual Voice Synthesis Engine for All Devices (Bhashini AI + Web Speech + Online Stream)
...
function playUniversalAudioStream(text, language, onStart, onEnd) {
  try {
    const langCode = language === 'marathi' ? 'mr' : (language === 'hindi' ? 'hi' : (language === 'gujarati' ? 'gu' : (language === 'bengali' ? 'bn' : (language === 'tamil' ? 'ta' : (language === 'telugu' ? 'te' : 'en-IN')))));
...
// Master speakInstruction function: Multi-Tier Universal Voice Dispatcher (Tier 1: Bhashini Voice AI)
export const speakInstruction = async (text, language = 'english', onStart = () => {}, onEnd = () => {}) => {
  ...
  // Tier 3: Check if browser already has an authentic voice for this language
  ...
  // Tier 4: Universal Online Audio Stream (Guaranteed crystal-clear Marathi & Hindi audio on any phone/PC)
  playUniversalAudioStream(text, language, onStart, onEnd);
};
...
// Speech-to-Text Recognition for Patient Voice Dictation
export const startSpeechRecognition = (language = 'hindi', onResult, onError, onEnd) => {
