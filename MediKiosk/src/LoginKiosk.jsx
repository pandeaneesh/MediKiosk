...
const LoginKiosk = ({ onOpenAdmin, onReturnToMenu }) => {
  ...
  // Post-Login Success State (Interactive Kiosk Session with persistent local storage)
  const [authenticatedPatient, setAuthenticatedPatient] = useState(() => {
    try {
      const saved = localStorage.getItem('medikiosk_active_patient_session');
  ...
  const handlePatientVerified = (patient) => {
    ...
    try {
      localStorage.setItem('medikiosk_active_patient_session', JSON.stringify(patient));
    } catch (e) {
      console.warn("Could not save patient session:", e);
    }
    speakInstruction(
      language === 'hindi' 
  ...
  const handleLogout = () => {
    sounds.playClick();
    try {
      localStorage.removeItem('medikiosk_active_patient_session');
    } catch (e) {
      // ignore
    }
