...
const AdminDashboard = ({ admin, onLogout, onReturnToMenu }) => {
  ...
  const handleRebalanceQueues = async () => {
    try {
      ...
      console.warn("Queue rebalance backend warning:", err);
    }
    setDoctorRoster(prev => prev.map(doc => {
      if (doc.waitingCount > 5) {
        return { ...doc, waitingCount: doc.waitingCount - 2 };
    ...
    showToast("⚡ Smart Queue Rebalance Triggered: Overflow diverted to Junior Resident chambers.");
  };
  ...
  const handleToggleDoctorStatus = async (docId) => {
    ...
    try {
      ...
      if (res?.status) nextStatus = res.status;
    } catch (err) {
      console.warn("Toggle doctor status backend warning:", err);
    }
    setDoctorRoster(prev => prev.map(doc => {
      if (doc.id === docId || doc.doctorId === docId) {
        const computedStatus = nextStatus || (doc.status === 'On Duty' ? 'In Emergency' : doc.status === 'In Emergency' ? 'On Break' : 'On Duty');
  ...
  const handleTestPrint = async (kioskId) => {
    let msg = `Test thermal slip dispensed at Kiosk ${kioskId}. Hardware verified.`;
    try {
      const res = await api.testPrintKiosk(kioskId);
      if (res?.message) msg = res.message;
    } catch (err) {
      console.warn("Kiosk test print backend warning:", err);
    }
