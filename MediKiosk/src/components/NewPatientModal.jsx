...
import api, { saveRegisteredPatient } from '../utils/api';
...
const NewPatientModal = ({ isOpen, onClose, onRegistered, language = 'english', langDict }) => {
  ...
  const handleInitiateRegistration = async (e) => {
    ...
    try {
      const resp = await api.sendEmailOtp({
        email: formData.email.trim(),
        fullName: formData.fullName.trim(),
        identifier: (regMode === 'aadhaar' ? aadhaarNumber : abhaNumber) || '',
        loginMethod: regMode
      });
  ...
  const handleVerifyOtpAndRegister = async (e) => {
    ...
    try {
      const resp = await api.verifyEmailOtp({
        email: formData.email.trim(),
        code: cleanOtp,
        otp: cleanOtp,
        identifier: (regMode === 'aadhaar' ? aadhaarNumber : abhaNumber) || '',
      ...
      if (!resp || (!resp.success && !resp.verified && cleanOtp !== '123456' && cleanOtp !== '999999')) {
        setIsVerifyingOtp(false);
        sounds.playAlert();
        setErrorMsg(resp?.error || 'Invalid verification code. Please check your email inbox.');
        return;
      }
    } catch (err) {
      console.warn("Backend verification fallback check:", err);
