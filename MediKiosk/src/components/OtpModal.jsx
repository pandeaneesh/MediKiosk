...
const OtpModal = ({ isOpen, onClose, identifier, method, patient, onVerified, langDict }) => {
  ...
  const handleResend = async () => {
    ...
    try {
      if (targetEmail && targetEmail.includes('@')) {
        await api.sendEmailOtp({ email: targetEmail, fullName: patientName, identifier, loginMethod: method });
      } else {
        const clean = identifier ? identifier.replace(/[\s-]/g, '') : '';
        await api.sendOtp({ identifier: clean, loginMethod: method, mobile: clean, email: targetEmail, fullName: patientName });
      }
    } catch (e) {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      if (patient?.email) {
        const emailRes = await api.verifyEmailOtp({ email: patient.email, code: fullOtp, identifier, loginMethod: method });
        if (emailRes && emailRes.patient) {
          serverPatient = emailRes.patient;
        }
      }
      if (!serverPatient) {
        const otpRes = await api.verifyOtp({ identifier, otp: fullOtp, code: fullOtp, loginMethod: method, email: patient?.email });
